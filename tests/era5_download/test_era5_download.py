...
# --- Missing field
@pytest.mark.parametrize("field", ["source_path", "start_date", "start_time", "end_date", "end_time", "delta_time", "variables", "levels"])
...
def test_config_parser_levels(base_config, levels, expected):
    ...
    parsed_config = config_parser(base_config)
    assert parsed_config["levels"] == expected, f"Expected levels to be {expected}, but got {parsed_config['levels']}"

@pytest.mark.parametrize("delta_time,expected", [
...
def test_config_parser_variables(base_config, variables, expected):
    ...
    parsed_config = config_parser(base_config)
    assert parsed_config["variables"] == expected, f"Expected variables to be {expected}, but got {parsed_config['variables']}"

# --- Custom save_name
@pytest.mark.parametrize("save_name", ["custom_name.nc", "era5_data.nc", "ERA5_data.nc"])
def test_config_parser_custom_save_name(base_config, save_name):
    """Test the custom save_name."""
    base_config["save_name"] = save_name
    parsed_config = config_parser(base_config)
    assert parsed_config["save_name"] == save_name, f"Expected save_name to be {save_name}, but got {parsed_config['save_name']}"
